# Agent Guide — ep_chat_log_join_leave
...
`ep_chat_log_join_leave` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_chat_log_join_leave`:
...
* Server: `eejsBlock_styles`, `userJoin`, `userLeave`
* Client: `chatNewMessage`
